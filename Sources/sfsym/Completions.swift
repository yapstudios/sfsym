...
_sfsym() {
    ...
    subs=(
        ...
        'categories:list Apple category keys'
        'colors:list named colors and hex values'
    ...
    case "$words[2]" in
        ...
        list)
            _arguments \
                ...
                '--contains[case-insensitive substring]:substring' \
                '--category[Apple category key]:key:->cats' \
                '--search[semantic keyword]:kw' \
                '--limit[cap count]:n' \
                '--json[JSON output]'
            case "$state" in
                cats)
                    local -a cats
                    cats=(${(f)"$(sfsym categories 2>/dev/null | awk '{print $1}')"})
