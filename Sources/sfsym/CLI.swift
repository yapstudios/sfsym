...
func runExportArgv(_ rawArgv: [String]) throws {
    ...
    // Stdout and unknown/missing extensions fall back to the default pdf.
    let knownExt: [String: String] = ["svg": "svg", "pdf": "pdf", "png": "png"]
    // Distinguish: no path / stdout → nil; known ext → known; unknown non-empty
    // ext → .unknown(raw). An empty pathExtension is "ambiguous, default to pdf"
    ...
    if let explicit = explicitFormat {
        ...
        case .unknown(let raw):
            FileHandle.standardError.write(Data("warning: unknown extension '.\(raw)' for -o; defaulting to pdf\n".utf8))
            format = "pdf"
        case .none:
            format = "pdf"
        }
        ...
        [
            ...
            "flags": [
                ["name": "--format", "short": "-f", "type": "enum",
                 ...
                 "description": "Output format. SVG and PDF are vector for mono/hier/palette; PDF multicolor embeds raster; SVG multicolor is not supported. When unset, inferred from the -o file extension (.svg/.pdf/.png); stdout or missing extensions default to pdf silently. If -o has a non-empty but unknown extension (e.g. .xyz), emits 'warning: unknown extension '.xyz' for -o; defaulting to pdf' on stderr and proceeds with pdf. If explicitly set and -o's known extension disagrees, emits 'warning: -f X contradicts -o .Y extension; writing X' on stderr and honors -f."],
                ["name": "--mode", "type": "enum",
