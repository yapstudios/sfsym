...
enum SvgEmitter {
    ...
    static func emit(
        ...
        for pe in painted {
            ...
            switch mode {
            case .monochrome:
                // Most monochrome glyphs get one flat path and this collapses
                // to modeAlpha = tintAlpha. But collapsed-template glyphs
                // (iphone, laptopcomputer, …) are drawn via the hierarchical
                // entry point and need per-path tier alphas to mirror what
                // NSImage.draw() produces for the PNG path. `hierarchyLevels`
                // is only passed through for that case — otherwise it's empty
                // and the tier lookup yields 0/alpha-1, same as before.
