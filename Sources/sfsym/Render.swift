...
enum Render {
    ...
    private static func renderPdf(options o: inout RenderOptions) throws -> (data: Data, glyph: Glyph) {
        ...
        // scale PNG uses), then offset by alignmentRect's inset so the glyph's
        // visible ink lands where NSImage.draw(in:) places it in the PNG path.
        // drawInContext: paints native pixel (0,0) → point (0,0) = rep's
        // lower-left; we shift that by (ar.minX, ar.minY) * fit so the
        // alignment inset becomes visible margin inside the box, matching
        // PNG's behavior.
        let box = o.pointSize
        ...
        let offX = (box - drawW) / 2 + ar.minX * fit
        let offY = (box - drawH) / 2 + ar.minY * fit
