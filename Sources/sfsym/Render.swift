...
enum Render {
    ...
    private static func renderPdf(options o: inout RenderOptions) throws -> (data: Data, glyph: Glyph) {
        ...
        // Canvas = pointSize × pointSize pt. Fit rep.size into the box (same
        // scale PNG uses). The offset matches what `sized.draw(in:)` does:
        //   - If alignmentRect's origin is inside rep.size (ar.minX/minY ≥ 0),
        //     that's padding between rep.size edge and the visible ink; add it
        //     so the ink ends up where NSImage places it.
        //   - If alignmentRect extends *past* rep.size (ar.minX/minY < 0), the
        //     extra is clipped by NSImage, so we don't add anything — clipping
        //     pins the artwork origin at rep.size's edge.
        // clamp(ar.min, 0, …) handles both branches uniformly. Bug surfaced on
        // shield.slash (ar.minX = -5.5) where adding the negative shifted the
        // symbol 17 px left of the PNG ground truth.
        let box = o.pointSize
        let ar = glyph.alignmentRect
        let fit = min(box / sz.width, box / sz.height)
        let drawW = sz.width * fit
        let drawH = sz.height * fit
        let offX = (box - drawW) / 2 + max(0, ar.minX) * fit
        let offY = (box - drawH) / 2 + max(0, ar.minY) * fit
        ...
        switch o.mode {
        case .monochrome:
            // Default NSImage.draw() renders device-style glyphs (iphone,
            // laptopcomputer, applewatch) with a translucent "screen" tier
            // even in monochrome mode — Apple silently reaches for the
            // hierarchical path whenever the glyph's `preferredRenderingMode`
            // is 3 (multicolor), which is the internal signal that the flat
            // template hides hierarchy. Match that so the SVG output looks
            // like the PNG output.
            //
            // `templateLayers < hierarchyLayers` alone is not enough:
            // convertible.side.air.fresh.fill has that same shape (1 vs 2)
            // but preferredRenderingMode=1 — its monochrome is flat black,
            // not hierarchical. Routing it through drawHierarchical paints
            // the whole car at tier-2 alpha 0.32 (wrong).
            if glyph.preferredRenderingMode == 3
                && glyph.numberOfTemplateLayers < glyph.numberOfHierarchyLayers {
    ...
    static func svg(options o: RenderOptions) throws -> Data {
        ...
        return SvgEmitter.emit(
            ...
            // monochrome renders that silently fell through to drawHierarchical
            // (collapsed-template glyphs — see the monochrome branch above).
            hierarchyLevels: opts.mode == .hierarchical
                || (opts.mode == .monochrome
                    && g.preferredRenderingMode == 3
                    && g.numberOfTemplateLayers < g.numberOfHierarchyLayers)
                ? g.hierarchyLevels : []
        )
    }
    ...
    static func png(options o: RenderOptions) throws -> Data {
        ...
        // predictably. IMPORTANT: must be .aqua (light mode) — under .darkAqua,
        // Apple's renderer silently mirrors several symbol families including
        // the "tophalf"/"bottomhalf"/"lefthalf"/"righthalf" variants (the filled
        // half swaps sides). SwiftUI / UIKit default to the aqua orientation, so
        // this is what users actually see when they use `Image(systemName:)`.
        NSAppearance(named: .aqua)!.performAsCurrentDrawingAppearance {
            NSGraphicsContext.saveGraphicsState()
