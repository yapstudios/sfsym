...
enum Render {
    ...
    static func svg(options o: RenderOptions) throws -> Data {
        ...
        var opts = o
        let (pdf, glyph) = try renderPdf(options: &opts)
        // Paths are extracted in the PDF's mediaBox coordinate space, which is
        // now the square canvas (pointSize × pointSize). The glyph itself has
        // already been translated + fit-scaled into this box by renderPdf.
        let box = opts.pointSize
        let canvas = CGSize(width: box, height: box)
        let paths = PdfToSvg.extractPaths(from: pdf, pageSize: canvas)
        // Use the post-cycling palette so SvgEmitter sees colors for every
        // layer index — otherwise trailing paths fall back to the PDF-lifted
        // fill (which is whatever CG emitted, not the user's color).
        ...
        return SvgEmitter.emit(
            ...
            hierarchyLevels: opts.mode == .hierarchical ? glyph.hierarchyLevels : []
        )
    }
