...
struct Glyph {
    ...
    // MARK: - Per-layer access (for SVG destination-out compositing)

    /// Metadata for one vector glyph layer — enough to decide whether it's a
    /// painted layer or an eraser (destination-out knockout mask) and in what
    /// order it composites.
    struct Layer {
        let name: String
        let index: Int
    ...
    /// Enumerate the palette layers' metadata. Palette and hierarchical layer
    /// arrays are the same underlying storage on CUINamedVectorGlyph — same
    /// geometry, same ordering; mode-specific interpretation is the colour
    /// resolver's job. This single list drives mono/hier/palette SVG emission
    /// so the eraser layer detection is consistent across modes.
    func paletteLayerList() -> [Layer] {
        let (fn, sel) = imp("paletteLayers",
            as: (@convention(c) (AnyObject, Selector) -> NSArray?).self)
        let arr = (fn(raw, sel) as? [AnyObject]) ?? []
        return arr.enumerated().map { (i, obj) -> Layer in
            let name = (obj.value(forKey: "name") as? String) ?? "layer-\(i)"
            let opacity = (obj.value(forKey: "opacity") as? Double) ?? 1.0
            let isEraser = (obj.value(forKey: "isEraserLayer") as? Bool) ?? false
            return Layer(name: name, index: i, opacity: opacity, isEraser: isEraser)
        }
    }
...
enum AppleSystemColor: String, CaseIterable {
    ...
    /// All raw names, in declaration order, for `sfsym colors` enumeration.
